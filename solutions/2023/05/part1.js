...
function findLocation(seed, mapsList) {
  mapsList.forEach(maps => {
    ...
    maps.forEach(({ source, destination, range }) => {
      if (!found && seed >= source && seed < source + range) {
...
function findLowestLocation(almanac) {
  ...
  const seedsList = seeds.split(': ')[1].split(' ').map(n => Number(n));
  const mapList = maps.map(mapToList);

  const locations = seedsList.map(seed => findLocation(seed, mapList));

  return Math.min(...locations);
}

findLowestLocation(sampleData); // => 35;
findLowestLocation(data); // => 389056265;
