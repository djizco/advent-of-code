...
const formatData = (data) => data.split(':')[1].trim().split(' ').filter(n => !!n).map(n => Number(n));

function calculateValue({ time, distance }) {
  for (let i = 0; i < time; i++) {
    const speed = i;
    const remainingTime = time - i;

    const newDistance = speed * remainingTime;
...
function calculateTimes(data) {
  const [timeData, distanceData] = data.split('\n');
  const times = formatData(timeData);
  const distances = formatData(distanceData);

  const races = times.map((time, i) => ({ time, distance: distances[i] }));
  const values = races.map(calculateValue);

  return values.reduce(R.multiply, 1);
}

calculateTimes(sampleData); // => 288
calculateTimes(data); // => 303600
